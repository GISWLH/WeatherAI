...
**Origin:** adapted from `GISWLH/WeatherLearn@feat/fengwu` into package `weatherai`  
...
See `papers/FENGWU_SPECS.md` and `weatherai.models.fengwu.channel_layout`.
...
Files under `/workspace/weatherai/hf_space_fengwu/` — parent uploads to Hugging Face.  
Buttons: unit tests, lite train smoke (ZeroGPU), tiny forward.

## License awareness

- WeatherAI package license: **MIT** (see `LICENSE` / `NOTICE`). Upstream WeatherLearn LICENSE file is MIT; some historical metadata mentioned BY-NC-SA — we follow the MIT LICENSE text for adapted code.
